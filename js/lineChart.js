...
class LineChart {
	...
	constructor(_parentElement, _estacion) {
	...
	wrangleData() {
		...
		vis.dataTimeFiltered = filteredData[vis.estacion].filter(d => {
	...
	updateVis() {
		...
		const newText = (vis.yValue === "price_usd") ? "Price ($)" 
			: (vis.yValue === "sensacion") ? "Market Capitalization ($)" 
